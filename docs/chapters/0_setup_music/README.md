# 0. Att lägga på musik

Att lägga på musik är enkelt, om du vet hur.

## 0.1. Att sätta på el

![](no_electricity_yet.jpg)

![](electricity.jpg)

## 0.2. Att hitta den rätta kabel

Öppna kabelboxen.

![Kabelboxen är sluten](cable_box.jpg)

Kabelboxen är nu öppet.

![Kabelboxen är öppet](opened_cable_box.jpg)

Tar en kabel. Den måsta har en stereo mini-jack på
ena sida, som passer i 'Audio in' ingången av stereon.

![En stereo mini-jack connector. Image från https://www.freeimageslive.co.uk/free_stock_image/stereo-jack-jpg, skapad av https://www.freeimageslive.co.uk/users/gratuit](stereo_mini_jack.jpg)

## 0.3. Plugga i din ljudkälla

Sticka din kabel i 'Audio in' ingången av stereon.

![Din kabel i 'Audio in' ingången av stereon](cable_in_audio_in.jpg)

Stick den andre änden av kabeln i din ljudkälla, t.ex. en mobiltelefon.

## 0.4. Sätt funktion till 'Audio in'

Tryck på 'Function' knappen till displayen visar 'Audio In'

![Tryck på 'Function' knappen till displayen visar 'Audio In'](set_function_to_audio_in.jpg)

## 0.5. Starta musik på din ljudkälla

Starta musik på din ljudkälla.

![Starta musik på din ljudkälla](start_music.jpg)

## 0.6. Ändra volymen

Ändra volymen för att får ljudinstallation att spela.

![Ändra volymen för att får ljudinstallation att spela](change_volume.jpg)
